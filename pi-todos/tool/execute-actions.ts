...
import { deriveTodoStatus, formatTickResult, serializeTodoForAgent, serializeTodoListForAgent, splitTodosByAssignment } from "../format.js";
...
async function resolveTodoRef(todosDir: string, params: { id?: string; title?: string }): Promise<{ id: string; filePath: string; todo: TodoRecord } | { error: string }> {
    if (params.id) {
    ...
    if (matches.length > 1) return { error: "Error: multiple todos share this title. Rename one todo first." };
...
export async function runGetAction(todosDir: string, params: { id?: string; title?: string }) {
    ...
    return { content: [{ type: "text" as const, text: serializeTodoForAgent(resolved.todo) }], details: { action: "get", todo: resolved.todo } };
}
...
export async function runAppendAction(todosDir: string, params: { id?: string; title?: string; body?: string }) {
    const resolved = await resolveTodoRef(todosDir, params);
    if ("error" in resolved) return error("append", resolved.error);
    if (!params.body || !params.body.trim()) return { content: [{ type: "text" as const, text: serializeTodoForAgent(resolved.todo) }], details: { action: "append", todo: resolved.todo } };
...
export async function runClaimAction(todosDir: string, params: { id?: string; title?: string; force?: boolean }, ctx: ExtensionCommandContext) {
    const resolved = await resolveTodoRef(todosDir, params);
...
export async function runReleaseAction(todosDir: string, params: { id?: string; title?: string; force?: boolean }, ctx: ExtensionCommandContext) {
    const resolved = await resolveTodoRef(todosDir, params);
...
export async function runTickAction(todosDir: string, params: { id?: string; title?: string; item?: string }) {
    const resolved = await resolveTodoRef(todosDir, params);
    if ("error" in resolved) return error("tick", resolved.error, { todo: undefined as never, remaining: [], allComplete: false });
    if (!params.item) return error("tick", "Error: item required for tick action", { todo: undefined as never, remaining: [], allComplete: false });
    const existing = resolved.todo;
    if (!existing.checklist?.length) return error("tick", "Error: Todo has no checklist. Use update action to add one.", { todo: existing, remaining: [], allComplete: false });
    const index = existing.checklist.findIndex(i => i.id === params.item);
    if (index === -1) return error("tick", `Error: Checklist item "${params.item}" not found in todo`, { todo: existing, remaining: [], allComplete: false });
