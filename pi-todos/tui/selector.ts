...
export class TodoSelectorComponent extends Container implements Focusable {
    ...
    constructor(tui: TUI, theme: Theme, todos: TodoFrontMatter[], onSelect: (todo: TodoFrontMatter) => void, onCancel: () => void, initialSearchInput?: string, currentSessionId?: string, onQuickAction?: (todo: TodoFrontMatter | null, action: TodoQuickAction) => void, onTab?: () => void, onCommand?: (action: "sweep-abandoned" | "sweep-completed") => void, mode: "open" | "closed" = "open") {
        super();
        this.tui = tui;
        this.theme = theme;
    ...
    private getSelectedItem(): TodoFrontMatter | null {
        if (this.selectedIndex === 0) return { id: CREATE_ITEM_ID, title: "", tags: [], status: "", created_at: "" };
    ...
    private renderState(): void {
        renderAll(this.tui, this.headerText, this.hintText, this.listContainer, this.theme, this.filteredTodos, this.selectedIndex, this.mode, this.currentSessionId);
    }
