...
export class TodoClosedMenuComponent extends Container {
    ...
    constructor(theme: Theme, todos: TodoRecord[], onSelect: (action: TodoClosedAction) => void) {
        ...
        this.addChild(new Text(theme.fg("dim", "↑↓ move • space select • enter delete selected • r reopen selected • a delete abandoned • c delete completed • esc back")));
        this.addChild(new DynamicBorder((s: string) => theme.fg("accent", s)));
        this.refresh();
    }
    ...
    private refresh(): void {
        ...
        for (let i = 0; i < this.todos.length; i += 1) {
            ...
            const statusText = status === "abandoned" ? this.theme.fg("error", "abandoned") : this.theme.fg("dim", "done");
            this.list.addChild(new Text(`${focus}${marked} ${this.theme.fg("accent", formatTodoId(todo.id))} ${todo.title} ${statusText}`));
        }
    }
    ...
    handleInput(keyData: string): void {
        ...
        if (kb.matches(keyData, "selectUp")) {
            this.selectedIndex = this.selectedIndex === 0 ? this.todos.length - 1 : this.selectedIndex - 1;
            this.refresh();
            return;
        }
        if (kb.matches(keyData, "selectDown")) {
            this.selectedIndex = this.selectedIndex === this.todos.length - 1 ? 0 : this.selectedIndex + 1;
            this.refresh();
            return;
        }
        if (keyData === " ") {
