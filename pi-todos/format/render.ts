...
export function renderTodoHeading(theme: Theme, todo: TodoFrontMatter, currentSessionId?: string): string {
    const derivedStatus = "checklist" in todo && todo.checklist?.length ? deriveTodoStatus(todo as TodoRecord) : getTodoStatus(todo);
...
export function renderTodoList(theme: Theme, todos: TodoFrontMatter[], expanded: boolean, currentSessionId?: string): string {
    ...
    const pushSection = (label: string, sectionTodos: TodoFrontMatter[]) => {
        ...
        for (let i = 0; i < maxItems; i++) lines.push(`  ${renderTodoHeading(theme, sectionTodos[i], currentSessionId)}`);
        if (!expanded && sectionTodos.length > maxItems) lines.push(theme.fg("dim", `  ... ${sectionTodos.length - maxItems} more`));
    };
...
export function renderChecklist(theme: Theme, checklist: ChecklistItem[]): string[] {
    ...
    for (const item of checklist) {
        const checkbox = item.status === "checked" ? theme.fg("success", "[x]") : theme.fg("dim", "[ ]");
