  ...
  for (const p of [globalPath, projectPath]) {
    ...
    try {
      ...
      if (parsed.inject && typeof parsed.inject === "object") {
        ...
        if (typeof parsed.inject.highThreshold === "number")
          base.inject.highThreshold = parsed.inject.highThreshold;
      }
    } catch {
...
function readMemories(store: Store): MemoryRow[] {
  ...
  const rows = db
    .prepare("SELECT id, content, timestamp, embedding FROM memories")
    .all() as MemoryRow[];
  db.close();
  return rows;
}
...
function listAllMemories(
  cwd: string,
  scope: Scope,
): Array<{ id: number; content: string; timestamp: string; source: string }> {
...
function deleteMemoryInStore(cwd: string, id: number, source: "global" | "project"): boolean {
  const store =
    source === "global"
      ...
      : ({ source: "project", dbPath: getProjectDbPath(cwd) } as Store);
...
async function openRememberManager(ctx: ExtensionCommandContext): Promise<void> {
  while (true) {
    ...
    const result = await ctx.ui.custom<RememberManagerAction>((tui, theme, _keybindings, done) => {
      ...
      container.addChild(
        new Text(
          theme.fg(
            "dim",
            `${all.length} memorie(s) • d delete • s search • a add • r refresh • i status`,
          ),
          2,
          0,
        ),
      ...
      const list = new SettingsList(
        visibleItems.length > 0
          ...
          : [
              {
                id: "none",
                label: theme.fg("dim", "No memories yet."),
                currentValue: "keep",
                values: ["keep"],
              },
            ],
      ...
      container.addChild(
        new Text(
          theme.fg(
            "dim",
            "↑↓ Navigate | d Delete | s Search | a Add | r Refresh | i Status | Esc Cancel",
          ),
          2,
          0,
        ),
        ...
        handleInput(data) {
          ...
          if (data === "d" || data === "D") {
            if (selected)
                ...
                type: "delete",
                id: selected.id,
                source: selected.source as "global" | "project",
              });
            return;
          }
          if (data === "s" || data === "S") {
    ...
    if (result.type === "search") {
      ...
      const lines = results
        .slice(0, 15)
        .map((r) => `id=${r.id} [${r.score.toFixed(3)}] [${r.source}] ${r.content}`);
    ...
    if (result.type === "add") {
      ...
      const global =
        config.scope === "both"
          ? await ctx.ui.confirm("Store", "Store in global scope? (No = project)")
          : config.scope === "global";
      const store = global
        ? ({ source: "global", dbPath: getGlobalDbPath() } as Store)
      ...
      const inserted = db
        ...
        .run(text.trim(), new Date().toISOString(), encodeEmbedding(emb));
      db.close();
    ...
    if (result.type === "delete") {
      const ok = await ctx.ui.confirm(
        "Forget memory",
        `Delete id=${String(result.id)} from ${result.source}?`,
      );
      if (!ok) continue;
      const deleted = deleteMemoryInStore(ctx.cwd, result.id, result.source);
      ctx.ui.notify(
        deleted
          ? `Forgot id=${String(result.id)} from ${result.source}.`
          : `Memory id=${String(result.id)} not found in ${result.source}.`,
...
export default function piRememberExtension(pi: ExtensionAPI): void {
  pi.registerCommand("remember", {
    ...
    handler: async (args, ctx) => {
      ...
      if (!trimmed) {
        if (!ctx.hasUI) {
          ctx.ui.notify(
            "Use /remember list | /remember search <query> | /remember forget <id>",
            "info",
          );
          return;
        }
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      const config = loadConfig(ctx.cwd);
      if (!config.enabled)
        return { content: [{ type: "text", text: "Remember plugin is disabled." }], details: {} };
      ...
      const stmt = db.prepare(
        "INSERT INTO memories (content, timestamp, embedding) VALUES (?, ?, ?)",
      );
      ...
      return {
        content: [
          {
            type: "text",
            text: `Remembered id=${String(result.lastInsertRowid)} [${store.source}] ${text}`,
          },
        ],
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      const lines = results
        .slice(0, limit)
        .map((r, i) => `${i + 1}. id=${r.id} [${r.score.toFixed(3)}] [${r.source}] ${r.content}`);
      return {
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      if (!fs.existsSync(store.dbPath))
        return { content: [{ type: "text", text: "No memory store found." }], details: {} };
      ...
      if (before.c === after.c)
        return {
          content: [{ type: "text", text: `Memory id=${params.id} not found in ${store.source}.` }],
          details: {},
        };
      return {
        content: [{ type: "text", text: `Forgot id=${params.id} from ${store.source}.` }],
        details: {},
      };
    },
  });
