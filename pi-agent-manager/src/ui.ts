...
export async function selectCwd(ctx: ExtensionContext): Promise<string> {
  ...
  while (true) {
    ...
    if (!query || query === "." || query === "./") {
      const confirm = await ctx.ui.select("No search query provided. Use project root?", ["Yes, use root", "No, search again"]);
      if (confirm === "Yes, use root") return ctx.cwd;
      continue;
    }

    let dirs: string[] = [];
    const findArgs = [".", "-maxdepth", "4", "-type", "d", "-not", "-path", "*/.*", "-iname", `*${query}*`];
    ...
    dirs = await new Promise((resolve) => {
      ...
      findProc.on("close", () => {
        resolve(buf.split("\n").map(d => d.trim()).filter(d => d && d !== "." && d !== "./"));
      });
    });

    if (dirs.length === 0) {
      const next = await ctx.ui.select(`No directories matching "${query}" found.`, ["Try Again", "Use Project Root", "Cancel"]);
      if (next === "Use Project Root" || next === "Cancel") return ctx.cwd;
      continue;
    }

    const options = ["-- Search Again --", ...dirs.map(d => d.startsWith("./") ? d : `./${d}`).slice(0, 50)];
...
export async function selectModel(ctx: ExtensionContext, current?: string): Promise<string | undefined> {
  const models = (ctx as any).modelRegistry.getAvailable().map((m: any) => `${m.provider}/${m.id}`).sort();
  const options = ["Default", ...models];
  const choice = await ctx.ui.select(current ? `Current Model: ${current}` : "Select Model", options);
...
export async function manageSubagents(pi: ExtensionAPI, ctx: ExtensionContext) {
  while (true) {
    ...
    const options = [...agents.map(a => `${disabledAgents.has(a.name) ? "❌" : "✅"} ${a.name}`), "---", "Create New Agent", "Back"];
    ...
    if (choice === "Create New Agent") await createAgent(pi, ctx);
      ...
      if (!agent) continue;
      const action = await ctx.ui.select(`Agent: ${name}`, ["Toggle Enable/Disable", "Change Model", "Manage Skill Permissions", "Edit Prompt", "Delete", "Back"]);
      if (action === "Toggle Enable/Disable") { 
        if (disabledAgents.has(name)) disabledAgents.delete(name); else disabledAgents.add(name); 
        savePermissions(ctx.cwd);
      }
      else if (action === "Change Model") {
        const newModel = await selectModel(ctx, agent.model);
        if (newModel !== undefined) { fs.writeFileSync(agent.filePath, serializeFrontmatter({ ...agent, model: newModel || undefined }) + "\n" + agent.systemPrompt); ctx.ui.notify(`Model for '${name}' updated.`, "info"); }
      } else if (action === "Manage Skill Permissions") {
        const allowedSkills = new Set<string>();
        if (agent.skillPermissions) for (const [s, act] of Object.entries(agent.skillPermissions)) if (act === "allow" && s !== "*") allowedSkills.add(s);
        ...
        fs.writeFileSync(agent.filePath, serializeFrontmatter({ ...agent, skillPermissions: newPermissions }) + "\n" + agent.systemPrompt);
        ...
        if (newPrompt !== undefined) fs.writeFileSync(agent.filePath, serializeFrontmatter(agent) + "\n" + newPrompt);
...
export async function createAgent(pi: ExtensionAPI, ctx: ExtensionContext) {
  ...
  if (!name) { ctx.ui.notify("Invalid agent name.", "error"); return; }
  const description = await ctx.ui.input("Short Description");
  if (!description) return;
  const model = await selectModel(ctx);
  if (model === undefined) return;
  const scope = await ctx.ui.select("Storage Location", ["Project (.pi/agents)", "Global (~/.pi/agent/agents)"]);
  if (!scope) return;
  const dir = scope === "Project (.pi/agents)" ? path.join(ctx.cwd, ".pi", "agents") : path.join(os.homedir(), ".pi", "agent", "agents");
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  const filePath = path.join(dir, `${name}.md`);
  if (fs.existsSync(filePath)) { ctx.ui.notify(`Agent already exists.`, "error"); return; }
  fs.writeFileSync(filePath, serializeFrontmatter({ name, description: "Placeholder.", model: model || undefined, skillPermissions: { "*": "deny" } }) + "\n\nPlaceholder prompt.");
...
export async function manageSkills(pi: ExtensionAPI, ctx: ExtensionContext, skillSet: Set<string>, title: string) {
  const piAny = pi as any;
  if (typeof piAny.getCommands !== "function") return;
  const allSkills = (piAny.getCommands() as any[]).filter(c => c.source === "skill").map(c => bareSkillName(c.name)).sort();
  while (true) {
    const options = [...allSkills.map(s => `${skillSet.has(s) ? "✅" : "❌"} ${s}`), "Back"];
    const choice = await ctx.ui.select(title, options);
    if (!choice || choice === "Back") break;
    const name = choice.replace(/^[✅❌]\s*/, "");
    if (skillSet.has(name)) skillSet.delete(name); else skillSet.add(name);
