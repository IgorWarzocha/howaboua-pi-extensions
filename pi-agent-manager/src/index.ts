...
export default function (pi: ExtensionAPI) {
    ...
    async execute(_id, params, signal, onUpdate, ctx) {
      ...
      const updateUI = () => {
        ...
        const fullOutput = taskStatuses
          .map((s) => `#### Output from ${s.name}\n${s.output || "(Waiting...)"}`)
          .join("\n\n");
        onUpdate({
          content: [
            { type: "text", text: `### Subagent Execution Dashboard\n${summary}\n\n${fullOutput}` },
          ],
        });
      };
      ...
      if (isParallel) {
        ...
        if (signal?.aborted)
          return { content: [{ type: "text", text: "Parallel batch aborted by user." }] };
        const resultsText = results
          .map(
            (r, i) =>
              `### Result [${i + 1}/${results.length}] from Subagent: ${r.name}\n\n${r.output}\n\n<session_resumption_id>\nsession_id: ${r.sessionId}\n</session_resumption_id>`,
          )
          .join("\n\n---\n\n");
  ...
  pi.registerCommand("subagent:abort", {
    ...
    handler: async (_args, ctx) => {
      ...
      for (const [pid, name] of activeSubagentProcesses.entries())
        try {
  ...
  for (const sub of allSubagents) {
    pi.registerCommand(`subagent:${sub.name}`, {
      ...
      handler: async (args, ctx) => {
        ...
        pi.sendUserMessage(
          `invoke_subagent(agent="${sub.name}", task="${task.replace(/"/g, '\\"')}")`,
          { deliverAs: "steer" },
        );
      },
    });
  ...
  pi.on("input", async (event, ctx) => {
    ...
    if (!agent || !task || disabledAgents.has(agent.name)) {
      return { action: "handled" };
    }
    pi.sendUserMessage(
      `invoke_subagent(agent="${agent.name}", task="${task.replace(/"/g, '\\"')}")`,
      { deliverAs: "steer" },
    );
    return { action: "handled" };
  });
