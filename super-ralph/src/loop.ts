...
export async function checkCondition(pi: ExtensionAPI, ctx: ExtensionContext, condition: string): Promise<boolean> {
    try {
...
export function stopLoop(pi: ExtensionAPI, ctx: ExtensionContext, state: RalphState | null, reason: string) {
    if (!state) return;
    state.active = false;
...
export async function triggerNextIteration(pi: ExtensionAPI, ctx: ExtensionContext, state: RalphState) {
    ...
    let prompt = "RALPH ITERATION " + state.iteration + (isReflection ? " | REFLECTION" : "") + "\n\n";
    prompt += "## Current Task (from " + state.taskFile + ")\n\n" + taskContent + "\n";

    if (isReflection) {
        prompt += "\n---\n## Reflection Turn\n";
        prompt += "Pause and reflect on your progress so far. Update the checklist and notes in " + state.taskFile + ".\n";
    }
