...
export default function (pi: ExtensionAPI) {
    ...
    pi.registerCommand("ralph", {
        ...
        handler: async (args, ctx) => {
            ...
            for (let i = 0; i < tokens.length; i++) {
                ...
                if (tok === "--until" && tokens[i+1]) { prefill.untilCondition = tokens[++i].replace(/^"|"$/g, ""); }
                else if (tok === "--prompt" && tokens[i+1]) { prefill.taskFile = tokens[++i].replace(/^"|"$/g, ""); }
                else if (!tok.startsWith("--")) { prefill.prompt = tok; }
            }
    ...
    pi.on("agent_end", async (event, ctx) => {
        ...
        const text = lastMsg && Array.isArray(lastMsg.content) 
