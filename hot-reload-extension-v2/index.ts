...
const DEFAULT_RESUME_PROMPT = "hot-reload worked correctly - you were implementing an extension - test it and continue troubleshooting if issues arise";
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      prompt: Type.Optional(
        Type.String({ description: "Optional continuation prompt sent after reload" }),
      ),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      return {
        content: [{ type: "text", text: "hot_reload_v2 sent /reload. Do NOT batch this tool with other calls; wait for resumed session." }],
